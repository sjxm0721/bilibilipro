...
import type { CommentData, PageComment, ReplyCommentInfo } from "@/api/comment/type";
...
const load = async ()=>{
  ...
  const res = await reqGetCommentPageList(pageInfo)
  commentList.value?.record.push.apply(commentList.value.record,res.data.record)
...
const refreshCommentList=async (totalFatherId:number)=>{
  ...
  const arr1:CommentData[]|undefined = commentList.value?.record.slice(0,(num-1)*pageInfo.pageSize)
  const arr3:CommentData[]|undefined = commentList.value?.record.slice(num*pageInfo.pageSize)
  pageInfo.page = num
  const res = await reqGetCommentPageList(pageInfo);
